...
import { Flame, Zap, Target, Clock, TrendingUp, Calendar, Award, BarChart2, BookOpen, Wand2, Menu } from "lucide-react";
import { roadmap } from "@/lib/data";
import { useMobileMenu } from "@/components/layout/DashboardShell";

interface ProfileStats {
    user: { name: string; email: string; image?: string | null; createdAt: string | Date };
    streak: { current: number; longest: number; total: number; lastActiveDate?: string | Date | null } | null;
    checklist: { completedKeys: string[]; completedDates: string[] };
    assignments: {
        total: number; completed: number; inProgress: number; skipped: number;
        totalTimeSpentMin: number; aiGenerated: number; completedDates: string[];
    };
    notes: { total: number; phaseNotes: number; itemNotes: number };
}
...
function ActivityHeatmap({ checklistDates, assignmentDates }: { checklistDates: string[]; assignmentDates: string[] }) {
    ...
    const { weeks, monthLabels } = useMemo(() => {
        ...
        for (let w = 0; w < 53; w++) {
            ...
            if (sunday.getDate() <= 7) monthLabels.push({ label: sunday.toLocaleDateString("en-IN", { month: "short" }), weekIdx: w });
        }
        ...
        <div style={{ overflowX: "auto", paddingBottom: 4 }}>
            <div style={{ position: "relative", height: 16, width: totalW, marginBottom: 6 }}>
                {monthLabels.map(({ label, weekIdx }) => (
                    <span key={weekIdx} style={{ position: "absolute", left: weekIdx * (CELL + GAP), fontSize: 9, color: "hsl(var(--muted-foreground))", whiteSpace: "nowrap" }}>{label}</span>
                ))}
            </div>
                    ...
                    <div key={wi} style={{ display: "flex", flexDirection: "column", gap: GAP }}>
                        {week.map((day) => (
                            <div key={day.date} title={`${day.date}: ${day.count}`}
                                style={{ width: CELL, height: CELL, borderRadius: 2, backgroundColor: cellColor(day.count), flexShrink: 0, cursor: "default" }} />
                        ))}
                    </div>
                ))}
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 4, justifyContent: "flex-end", marginTop: 10 }}>
                ...
                {[0, 0.2, 0.5, 0.8, 1].map((v, i) => (
                    <div key={i} style={{ width: CELL, height: CELL, borderRadius: 2, flexShrink: 0, backgroundColor: cellColor(v === 0 ? 0 : Math.ceil(v * maxCount)) }} />
                ))}
            ...
            {roadmap.map((phase, pi) => {
                ...
                return (
                    <div key={pi} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <span style={{ fontSize: 10, fontWeight: 600, width: 24, flexShrink: 0, color: phase.color }}>{phase.phase}</span>
                        <div style={{ flex: 1, height: 5, backgroundColor: "rgba(255,255,255,0.07)", borderRadius: 3, overflow: "hidden" }}>
                            <div style={{ height: "100%", width: `${pct}%`, backgroundColor: phase.color, borderRadius: 3, transition: "width 0.7s ease" }} />
                        </div>
                        <span style={{ fontSize: 9, color: "hsl(var(--muted-foreground))", width: 68, textAlign: "right", flexShrink: 0 }}>{done}/{total} · {pct}%</span>
                    </div>
                );
            })}
        </div>
...
function StatCard({ icon: Icon, label, value, sub, color }: {
    ...
    return (
        <div style={{ border: "1px solid hsl(var(--border))", borderRadius: 6, padding: "10px 12px", backgroundColor: "hsl(var(--card))", display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 700, lineHeight: 1, color }}>{value}</div>
            {sub && <div style={{ fontSize: 9, color: "hsl(var(--muted-foreground))" }}>{sub}</div>}
        </div>
    );
}

function WeeklyActivity({ checklistDates, assignmentDates }: { checklistDates: string[]; assignmentDates: string[] }) {
    const weeks = useMemo(() => {
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ display: "flex", gap: 4, height: 96, alignItems: "flex-end" }}>
                {weeks.map((w, i) => {
                    ...
                    return (
                        <div key={i} title={`${w.label}: ${w.checklist} items, ${w.assignments} asgns`}
                            style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "flex-end", gap: 1, height: "100%", cursor: "default" }}>
                            {asgnPct > 0 && <div style={{ height: `${asgnPct}%`, minHeight: 2, backgroundColor: "#a855f7", borderRadius: "2px 2px 0 0", flexShrink: 0 }} />}
                            {clPct > 0 && <div style={{ height: `${clPct}%`, minHeight: 2, backgroundColor: "#22c55e", borderRadius: asgnPct > 0 ? 0 : "2px 2px 0 0", flexShrink: 0 }} />}
                            {total === 0 && <div style={{ height: 3, backgroundColor: "rgba(255,255,255,0.06)", borderRadius: 2, flexShrink: 0 }} />}
                        </div>
                    );
                })}
            </div>
            ...
            <div style={{ display: "flex", gap: 16 }}>
                {[{ color: "#22c55e", label: "Roadmap items" }, { color: "#a855f7", label: "Assignments" }].map(({ color, label }) => (
...
function CircleProgress({ pct }: { pct: number }) {
    ...
    return (
        <div style={{ position: "relative", width: 64, height: 64, flexShrink: 0 }}>
            <svg viewBox="0 0 64 64" width={64} height={64} style={{ transform: "rotate(-90deg)", display: "block" }}>
...
export default function ProfileClient({ stats }: { stats: ProfileStats }) {
    ...
    const card: React.CSSProperties = { border: "1px solid hsl(var(--border))", borderRadius: 8, padding: 16, backgroundColor: "hsl(var(--card))" };
    const sTitle: React.CSSProperties = { fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.12em", color: "hsl(var(--muted-foreground))" };

    return (
        <div style={{ minHeight: "100vh" }}>

            {/* Header */}
            <div style={{ position: "sticky", top: 0, zIndex: 10, backgroundColor: "hsl(var(--background))", borderBottom: "1px solid hsl(var(--border))", padding: "12px 16px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <button onClick={openMobileMenu} className="md:hidden"
                        style={{ display: "flex", alignItems: "center", padding: 4, borderRadius: 4, border: "none", background: "none", cursor: "pointer", color: "hsl(var(--muted-foreground))", marginLeft: -4 }}>
                        <Menu style={{ width: 16, height: 16 }} />
                    </button>
                    <div>
                        <p style={{ fontSize: 10, letterSpacing: "0.2em", color: "hsl(var(--muted-foreground))", textTransform: "uppercase" }}>backend mastery</p>
                        <h1 style={{ fontSize: 18, fontWeight: 600, marginTop: 2 }}>Profile</h1>
                    </div>
                </div>
            </div>

            <div style={{ padding: "16px", maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 12 }}>

                {/* User card */}
                <div style={{ ...card, display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                    {user.image ? (
                        <img src={user.image} alt={user.name} style={{ width: 48, height: 48, borderRadius: "50%", border: "1px solid hsl(var(--border))", objectFit: "cover", flexShrink: 0 }} />
                    ) : (
                        <div style={{ width: 48, height: 48, borderRadius: "50%", border: "1px solid hsl(var(--border))", backgroundColor: "hsl(var(--secondary))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 700, color: "hsl(var(--muted-foreground))", flexShrink: 0 }}>
                            {getInitials(user.name)}
                        </div>
                    )}
                    <div style={{ flex: 1, minWidth: 0 }}>
                        <h2 style={{ fontSize: 15, fontWeight: 600 }}>{user.name}</h2>
                        ...
                        <div style={{ marginTop: 6, display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                            {(streak?.current ?? 0) > 0 && <span style={{ fontSize: 11, color: "#f97316" }}>🔥 {streak!.current}-day streak</span>}
                            <span style={{ fontSize: 10, color: "hsl(var(--muted-foreground))" }}>{checklist.completedKeys.length}/{totalItems} items done</span>
                        </div>
                    </div>
                ...
                {/* Stats grid — 2 cols mobile, 3 sm, 6 lg */}
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
                    <StatCard icon={Flame} label="Streak" value={streak?.current ?? 0} sub={`${streak?.longest ?? 0} longest`} color="#f97316" />
                    <StatCard icon={Target} label="Completed" value={checklist.completedKeys.length} sub={`of ${totalItems}`} color="#22c55e" />
                    <StatCard icon={Zap} label="Assignments" value={assignments.completed} sub={`of ${assignments.total}`} color="#3b82f6" />
                    <StatCard icon={Clock} label="Time Spent" value={formatMinutes(assignments.totalTimeSpentMin)} sub={avgTime ? `avg ${formatMinutes(avgTime)}` : undefined} color="#a855f7" />
                    <StatCard icon={BookOpen} label="Notes" value={notes.total} sub={`${notes.phaseNotes}ph · ${notes.itemNotes}it`} color="#eab308" />
                    <StatCard icon={Wand2} label="AI" value={assignments.aiGenerated} sub="generated" color="#ec4899" />
                </div>
                ...
                <div style={card}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            ...
                            <span style={sTitle}>Activity — Last 365 Days</span>
                        </div>
                        <span style={{ fontSize: 10, color: "hsl(var(--muted-foreground))" }}>{streak?.total ?? 0} active days</span>
                    </div>
                    <ActivityHeatmap checklistDates={checklist.completedDates} assignmentDates={assignments.completedDates} />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div style={card}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                            ...
                            <span style={sTitle}>Weekly — Last 12 Weeks</span>
                        </div>
                        <WeeklyActivity checklistDates={checklist.completedDates} assignmentDates={assignments.completedDates} />
                    </div>
                    <div style={card}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                            <TrendingUp style={{ width: 14, height: 14, color: "hsl(var(--muted-foreground))" }} />
                            <span style={sTitle}>Phase Progress</span>
                        </div>
                ...
                <div style={{ ...card, display: "flex", flexDirection: "column", gap: 14 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={sTitle}>Assignment Breakdown</span>
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        {[
                            ...
                            <div key={label} style={{ border: "1px solid hsl(var(--border))", borderRadius: 6, padding: "10px 12px", textAlign: "center" }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div style={{ ...card, display: "flex", flexDirection: "column", gap: 14 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            ...
                            <span style={sTitle}>Streak Details</span>
                        </div>
                        ...
                        {streak?.lastActiveDate && (
                            <p style={{ fontSize: 10, color: "hsl(var(--muted-foreground))" }}>Last active: {formatDate(streak.lastActiveDate)}</p>
                        )}
                    </div>
                    ...
                    <div style={{ ...card, display: "flex", flexDirection: "column", gap: 10 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            ...
                            <span style={sTitle}>Achievements</span>
                        </div>
                        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                            {achievements.map(({ label, desc, unlocked, icon }) => (
                                <div key={label} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", border: "1px solid hsl(var(--border))", borderRadius: 6, opacity: unlocked ? 1 : 0.35 }}>
                                    ...
                                    {unlocked && <div style={{ width: 6, height: 6, borderRadius: "50%", backgroundColor: "#22c55e", flexShrink: 0 }} />}
                                </div>
                            ))}
                        </div>
                    </div>
