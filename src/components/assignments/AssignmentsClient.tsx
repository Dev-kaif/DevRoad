...
import { Plus, Wand2, Play, CheckCircle, Clock, Loader2, Trash2, Timer, Key, Eye, EyeOff, X, Menu } from "lucide-react";
...
// Props no longer needs onMobileMenuOpen — use context
interface Props {
    initialAssignments: Assignment[];

}

export default function AssignmentsClient({ initialAssignments }: Props) {
    ...
    async function generateWithAI() {
        if (!geminiKey) { setKeyDialogOpen(true); return; }
        setGenerating(true);
        try {
...
Return ONLY valid JSON, no markdown, no extra text:
{
  ...
  "phaseName": "${phase?.title ?? "General"}"
    ...
    async function completeAssignment(id: string) {
        ...
        await updateAssignment(id, { status: "completed", completedAt: new Date().toISOString(), timeSpentMin: spent });
        if (activeTimer?.id === id) { setActiveTimer(null); setElapsed(0); }
        ...
        <div className="min-h-screen">
            ...
            <div className="sticky top-0 z-10 bg-background border-b border-border px-4 sm:px-6 py-3 sm:py-4">
                <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                            <p className="text-[10px] tracking-[0.2em] text-muted-foreground uppercase hidden sm:block">30-day grind</p>
                            <h1 className="text-base sm:text-lg font-semibold leading-tight">Assignments</h1>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                        <button
                            ...
                            className={cn(
                                "hidden sm:flex items-center gap-1.5 text-[10px] px-2.5 py-1 rounded-full border transition-all",
                                geminiKey
                        ...
                        >
                            ...
                            {geminiKey ? "Gemini ready" : "Add key"}
                        </button>

                        {/* Mobile key indicator */}
                        <button onClick={() => setKeyDialogOpen(true)} className="sm:hidden">
                            <Key className={cn("w-4 h-4", geminiKey ? "text-green-400" : "text-muted-foreground")} />
                        </button>
                        ...
                        <Button size="sm" className="text-xs h-7 sm:h-8 px-2.5 sm:px-3" onClick={() => setCreateOpen(true)}>
                ...
                {activeTimer && (() => {
                    ...
                    return (
                        <div className={cn(
                            "mt-2.5 flex items-center gap-2 sm:gap-3 px-3 py-2 rounded border text-sm",
                            ...
                            <span className="text-xs opacity-70 hidden sm:inline">/ {a?.timeLimitMin}m</span>
                            <span className="text-xs flex-1 truncate opacity-70">{a?.title}</span>
                            {over && <span className="text-xs font-semibold hidden sm:inline">OVER TIME</span>}
                            <Button size="sm" variant="outline" className="h-6 text-[10px] shrink-0"
                                onClick={() => completeAssignment(activeTimer.id)}>
                ...
                <div className="flex gap-1 mt-2.5 overflow-x-auto scrollbar-none">
                    {["all", "pending", "in_progress", "completed", "skipped"].map((s) => (
                        <button key={s} onClick={() => setFilter(s)}
                            className={cn(
                                "text-[10px] px-2 sm:px-2.5 py-1 rounded border transition-colors uppercase tracking-wider shrink-0",
                ...
                {filtered.map((assignment) => {
                    ...
                    return (
                        <div key={assignment.id} className={cn(
                            "border rounded-md p-3 sm:p-4 group transition-all",
                            ...
                            <div className="flex items-start gap-3">
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 flex-wrap mb-1">
                                        ...
                                        {phase && (
                                            <span className="text-[10px] hidden sm:inline" style={{ color: phase.color + "aa" }}>
                                    ...
                                    <h3 className="text-xs sm:text-sm font-medium leading-snug">{assignment.title}</h3>
                                    <p className="text-[11px] sm:text-[12px] text-muted-foreground mt-1 leading-relaxed whitespace-pre-wrap">
                ...
                <DialogContent className="max-w-sm bg-card border-border">
                    ...
                    <div className="space-y-4">
                        <p className="text-[11px] text-muted-foreground leading-relaxed">
                            Your key is saved in <strong className="text-foreground">your browser&apos;s localStorage only</strong> — it never touches our servers.
                        </p>
                        ...
                        <div className="border border-border rounded-md p-3 space-y-2">
                            <div className="flex items-center justify-between">
                                ...
                                {geminiKey ? (
                                    ...
                                    <button onClick={() => { setCreateOpen(false); setKeyDialogOpen(true); }}
                            ...
                            <Button variant="outline" size="sm" className="text-xs w-full" onClick={generateWithAI} disabled={generating}>
                                {generating ? <Loader2 className="w-3 h-3 mr-1.5 animate-spin" /> : <Wand2 className="w-3 h-3 mr-1.5" />}
                        ...
                        <div className="space-y-3">
                            <div className="space-y-1">
                                ...
                                <Input value={title} onChange={(e) => setTitle(e.target.value)}
                                    placeholder="Build a JWT refresh token system"
