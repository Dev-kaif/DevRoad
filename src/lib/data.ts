...
export const roadmap: RoadmapPhase[] = [
    ...
    },
    {
        ...
        duration: "Week 26",
        color: "#00ff88",
        why: "Interviewers will ask deep questions about your own projects. You need to own every line, every decision.",
        sections: [
            {
                title: "Go Deep on Each Project",
                items: [
                    { text: "Beatflow — explain the music generation pipeline end-to-end, every tech decision", build: false },
                    { text: "ViralRot — explain the distributed GPU pipeline, how jobs are queued and processed", build: false },
                    { text: "OpenFlowX — explain the event-driven workflow engine, execution guarantees", build: false },
                    { text: "SketchWiz — explain the WebSocket room system, state sync, conflict resolution", build: false },
                    { text: "For each project: what would you do differently?", build: false },
                    { text: "For each project: how would you scale it to 10x current load?", build: false },
                ],
            },
            {
                title: "Harden Your Projects",
                items: [
                    { text: "Add structured Pino logging with correlation IDs to your best project", build: true },
                    { text: "Add /health endpoints, Sentry error tracking, proper graceful shutdown", build: true },
                    { text: "Run EXPLAIN ANALYZE on real queries — fix at least one slow query", build: true },
                    { text: "Add rate limiting to public endpoints", build: true },
                    { text: "Review all auth flows — are refresh tokens rotating? Are cookies httpOnly?", build: true },
                ],
            },
            {
                title: "The No-AI Build",
                items: [
                    { text: "Build one small but complete project with zero AI assistance — just docs", build: true },
                    { text: "Ideas: JWT auth service, rate limiter npm package, job queue with BullMQ, URL shortener", build: true },
                    { text: "Time yourself — what takes longest tells you where to drill more", build: true },
                    { text: "Write a blog post or README explaining every technical decision", build: true },
                ],
            },
        ],
    },
